...
namespace ShoppingBLLibrary
{
    public class CustomerBL: ICustomerService
    {
        ...
        public async Task<int> AddCustomer(string customerName)
        {
            int customerId = _customerRepo .GetLatestId() + 1;
            int cartId = _cartRepo.GetLatestId() + 1;  
            int ordersId = _orderRepo.GetLatestId() + 1;

            await cartService.AddCartToCustomer(cartId, new Cart(cartId, new List<Product>()));
            await orderService.AddOrdersToCustomer(ordersId, new List<Order>());

            Customer customer = new Customer(customerId, customerName, cartId, ordersId);
            _customerRepo.Add(customerId, customer);
            Console.WriteLine("latest id " + _customerRepo.GetLatestId());
            return customerId;
        }
        ...
        {
            Customer customer = await _customerRepo.GetById(customerId);
            if (customer == null)
            {
